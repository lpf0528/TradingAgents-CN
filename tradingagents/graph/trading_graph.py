...
class TradingAgentsGraph:
    ...
    def __init__(
        ...
        if normalized_quick_provider and normalized_deep_provider and normalized_quick_provider != normalized_deep_provider:
            ...
            custom_api_key = (
                self.config.get("custom_openai_api_key")
            ...
            if not custom_api_key:
                raise ValueError("使用自定义OpenAI端点需要设置CUSTOM_OPENAI_API_KEY或OPENAI_API_KEY环境变量")

            custom_base_url = (
                self.config.get("custom_openai_base_url")
                or (self.config.get("backend_url") if self.config.get("backend_url") != "https://api.openai.com/v1" else None)
                or os.getenv("CUSTOM_OPENAI_BASE_URL")
                or os.getenv("OPENAI_BASE_URL")
                or "https://tenrouter.weike.fm/v1"
            )
