...
from tradingagents.utils.async_utils import safe_run_async
...
class DataSourceManager:
    ...
    def get_stock_data(self, symbol: str, start_date: str = None, end_date: str = None, period: str = "daily") -> str:
        ...
        try:
            ...
            if is_success:
                ...
                # 数据质量异常时也尝试降级到其他数据源
                fallback_res = self._try_fallback_sources(symbol, start_date, end_date)
                fallback_str = fallback_res[0] if isinstance(fallback_res, tuple) else fallback_res
                if fallback_str and "❌" not in fallback_str and "错误" not in fallback_str:
                    logger.info(f"✅ [数据来源: 备用数据源] 降级成功获取数据: {symbol}")
                    return fallback_str
                else:
                    logger.error(f"❌ [数据来源: 所有数据源失败] 所有数据源都无法获取有效数据: {symbol}")
                    return fallback_str if fallback_str else (result if isinstance(result, str) else str(result))
            ...
            fallback_res = self._try_fallback_sources(symbol, start_date, end_date)
            return fallback_res[0] if isinstance(fallback_res, tuple) else fallback_res
    ...
    def _get_tushare_data(self, symbol: str, start_date: str, end_date: str, period: str = "daily") -> str:
        ...
        try:
            ...
            # 使用异步方法获取历史数据
            data = safe_run_async(lambda: provider.get_historical_data(symbol, start_date, end_date))
    ...
    def _get_akshare_data(self, symbol: str, start_date: str, end_date: str, period: str = "daily") -> str:
        ...
        try:
            ...
            # 使用异步方法获取历史数据
            data = safe_run_async(lambda: provider.get_historical_data(symbol, start_date, end_date, period))
