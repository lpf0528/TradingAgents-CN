    ...
    def _get_real_financial_metrics(self, symbol: str, price_value: float) -> dict:
        ...
        try:
            ...
            if akshare_provider.connected:
                # AKShare的get_financial_data是异步方法，需要使用safe_run_async运行
                financial_data = safe_run_async(lambda: akshare_provider.get_financial_data(symbol))
                ...
                if financial_data and any(not v.empty if hasattr(v, 'empty') else bool(v) for v in financial_data.values()):
                    ...
                    # 获取股票基本信息（也是异步方法）
                    stock_info = safe_run_async(lambda: akshare_provider.get_stock_basic_info(symbol))
            ...
            # 获取财务数据（异步方法）
            financial_data = safe_run_async(lambda: provider.get_financial_data(symbol))
